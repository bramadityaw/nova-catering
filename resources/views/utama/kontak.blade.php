            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-12">
                    ...
                    <form action="#" method="POST" class="d-flex flex-wrap justify-content-between">
                        ...
                        <button type="submit" class="banner-btn style-2" style="margin-bottom: 60px;">Kirim Pesan</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const form = document.querySelector("form"); // Sesuaikan dengan selektor form

        form.addEventListener("submit", function(e) {
            e.preventDefault(); // Mencegah form agar tidak langsung submit

            // Ambil data dari input form dan hapus spasi di akhir
            const nama_pengirim = form.querySelector("input[placeholder='Nama Anda']").value.trim();
            const subjek = form.querySelector("input[placeholder='Subjek/Pertanyaan']").value.trim();
            // Gantikan newline dengan %0A untuk keperluan URL encoding
            const isi_pesan = form.querySelector("textarea[placeholder='Pesan Anda']").value.trim().replace(/\n/g, '%0A');

            // Format pesan WhatsApp
            const nomorWhatsapp = "6281389785631";
            const pesan = `Assalamu'alaikum Ibu,%0A` +
                `Saya *${nama_pengirim}*,%0A` +
                `*${subjek}*%0A` +
                `${isi_pesan}`;

            // Buat URL untuk membuka WhatsApp Web
            const urlWhatsapp = `https://wa.me/${nomorWhatsapp}?text=${pesan}`;

            // Buka WhatsApp Web dengan pesan yang telah diformat
